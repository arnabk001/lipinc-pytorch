...
def get_result(input_path,advanced_folder_path,checkpoint_path):
  ...
  try:
    ...
    if length_error:
      ...
      combined_frames = torch.tensor(combined_frames, dtype=torch.float32).to(device)
      residue_frames = torch.tensor(residue_frames, dtype=torch.float32).to(device)

      combined_frames = combined_frames.permute(0, 4, 1, 2, 3)  # Convert to (batch, channels, depth, height, width)
      residue_frames = residue_frames.permute(0, 4, 1, 2, 3)
      ...
      with torch.no_grad():
        result = model(combined_frames, residue_frames)
...
#   device = args.device
  ...
  out = cv2.VideoWriter(demo_path,cv2.VideoWriter_fourcc(*'mp4v'), 21, frameSize)
